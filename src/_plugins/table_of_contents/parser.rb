module Jekyll
  module TableOfContents
    ...
    class Parser
      ...
      def initialize(html, doc_url = "", options = {})
      ...
      def build_toc
        %(<#{@list_type} class="section-nav">\n#{build_toc_list(@entries, last_ul_used: true)}</#{@list_type}>)
      end

      def inject_anchors_into_html
        @entries.each do |entry|
          entry[:content_node].add_previous_sibling(%(<a id="#{entry[:id]}#{entry[:uniq]}" class="anchor" href="#{@doc_url}##{entry[:id]}#{entry[:uniq]}" aria-hidden="true"><span class="octicon octicon-link"></span></a>))
        end
      ...
      def build_toc_list(entries, last_ul_used: false)
        ...
        while i < entries.count
          ...
          if entry[:h_num] == min_h_num
            ...
            toc_list << %(<li class="toc-entry toc-#{entry[:node_name]}"><a href="#{@doc_url}##{entry[:id]}#{entry[:uniq]}">#{entry[:text]}</a>)
            ...
            if i + 1 < entries.count
              ...
              if next_entry[:h_num] > min_h_num
                ...
                toc_list << %(\n<#{@list_type}>\n#{build_toc_list(nest_entries, last_ul_used: true)}</#{@list_type}>\n)
